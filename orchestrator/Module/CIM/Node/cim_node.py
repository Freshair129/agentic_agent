...
class ContextInjectionModule:
    ...
    def inject_phase_1(
        ...
        engram_hit: Optional[Dict[str, Any]] = None  # [NEW]
    ) -> Dict[str, Any]:
        """
...
## ⚡ REFLEX_CANDIDATE: [SYSTEM 1 ENGRAM HIT]
{'' if not context.get('engram_hit') else f"**Cached Response:** {context['engram_hit'].get('context_data', {}).get('response', 'N/A')}\n**Hit Confidence:** {context['engram_hit'].get('confidence', 0):.2f}" if context.get('engram_hit') else 'No direct reflex match.'}
> **Note:** This is a cached response from a similar past interaction. VERIFY if it is still appropriate for the current context.
