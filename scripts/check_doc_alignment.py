...
def check_documentation_alignment():
    ...
    try:
        with open(REGISTRY_PATH, 'r', encoding='utf-8') as f:
            ...
            print(f"   ℹ️  Organizer Version (SSOT): v{truth_version}")
    ...
    version_pattern = re.compile(r"v\s*(\d+\.\d+\.\d+)", re.IGNORECASE)
    ...
    for relative_path in CRITICAL_DOCS:
        ...
        # Heuristic: Check the first 10 lines for "Version: X.X.X" or "vX.X.X" header
        header_chunk = content[:500] 
        match = version_pattern.search(header_chunk)
        ...
        if match:
            ...
            if doc_version != truth_version:
                ...
                print(f"   ✅ {relative_path} matches v{doc_version}")
        else:
